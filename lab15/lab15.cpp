...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		std::string p = getSize() + " MAC from function (dynamic)";
		...
		if (hLib)
		{
			...
			if (log)
			{				
				 std::string p0 = log->write() + " MAC from class (dynamic)";
				 std::wstring stemp0= std::wstring(p0.begin(), p0.end());
				 LPCWSTR sw0 = stemp0.c_str();
				MessageBox(hWnd, sw0, L"neError", NULL);
			}
			
		}
		FreeLibrary(hLib);

		std::string p1 = getSize2() + " MAC from function (static)";
		...
		std::string p2 = log2->write2() + " MAC from class (static)";
